...
mv_gcc10=false; tag_buildroot=24;
...
case $build_name in
	...
	cn9130 ) device_tree="cn9130-db-A";
		...
		kernel_dtb=$kernel_dtb"marvell/cn9130-crb-A marvell/cn9131-db ";
		;;
	...
	cn9130_crb_C | cn9130_crb-r1p3 | cn9130_crb_B-r1p3 | cn9130_crb_C-r1p3 )
		if [[ "$tag_kernel" == "6.12" ]]; then
			case $build_name in
				cn9130_crb_C | cn9130_crb_C-r1p3 )
					echo "Error: $build_name board not supported with 6.12 kernel";
					exit 1
					;;
				cn9130_crb-r1p3 ) device_tree="cn9130-crb-A"; ;;
				cn9130_crb_B-r1p3 ) device_tree="cn9130-crb-B"; ;;
			esac
		else
			case $build_name in
		...
		;;
	cn9131 ) device_tree="cn9131-db-A"; kernel_dtb="marvell/cn9131-db ";cp_num="CP_NUM=2"; uefi_plat="t9131"; ;;
	cn9131_B ) device_tree="cn9131-db-B"; cp_num="CP_NUM=2"; uefi_plat="t9131"; ;;
	cn9131_C ) device_tree="cn9131-db-C"; cp_num="CP_NUM=2"; uefi_plat="t9131"; ;;
	cn9132 ) device_tree="cn9132-db-A"; kernel_dtb="marvell/cn9132-db ";cp_num="CP_NUM=3"; uefi_plat="t9132"; ;;
	cn9132_B ) device_tree="cn9132-db-B"; cp_num="CP_NUM=3"; uefi_plat="t9132"; ;;
	cn9132_C ) device_tree="cn9132-db-C"; cp_num="CP_NUM=3"; uefi_plat="t9132"; ;;
	cn81* ) device_tree=""; ;;
	cn83* ) device_tree=""; ;;
	cn96* ) device_tree=""; ;;
	cn93* ) device_tree=""; ;;
...
if [[ $boot_uefi ]]; then
	if [[ ${atf_plat} != "t9130" ]]; then
		CONFIG_SCRIPT_TARGET_UEFI_PLATFORM="${config_script} --set-str TARGET_UEFI_PLATFORM ${atf_plat}"
	else
		CONFIG_SCRIPT_TARGET_UEFI_PLATFORM="${config_script} --set-str TARGET_UEFI_PLATFORM ${uefi_plat}"
	fi
else
...
#Enable OPTEE
if [ $optee_used ]; then
	if [[ $atf_plat == "t96" || $atf_plat == "f95" || $atf_plat == "t98" || $soc_family == "cn10k" || $soc_family == "cn20k" ]]; then
	config_cmd=$config_cmd"""
...
if [[ $xtend_name == "ml" || $xtend_name =~ *-ml || $xtend_name =~ ml-* ]]; then
	if [[ $build_name =~ cn10k* || $build_name =~ cnf10k* || $build_name =~ cn20k* || $build_name =~ cnf20k* ]]; then
		config_cmd=$config_cmd"""
		${config_script} --set-str BR2_TARGET_MARVELL_ML_ARCH \"${build_name}\"
		"""
	fi
fi
...
if [ "${BUILD_MCP:-}" ]; then
config_cmd=$config_cmd"""
	${config_script} --enable BR2_MARVELL_EXTERNAL_FW_MCP_BUILD
	"""
echo "MCP_BUILD is set to true in the environment."
fi
